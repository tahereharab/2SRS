...
class RankAggregator(object):
    ...
    def average_rank(self, rank_list):
        ...
        return sorted(agg_dict.items(), key=itemgetter(1), reverse=False) #sort_by_value(agg_dict)
    ...
    def _get_count_dict(self, rank_list):
        ...
        return sorted(counts.items(), key=itemgetter(1), reverse=True) #sort_by_value(counts, reverse=True)
    ...
    def borda(self, rank_list, min_score=1, unranked='split'):
        ...
        return sorted(scores.items(), key=itemgetter(1), reverse=True)#sort_by_value(scores, reverse=True)
